...
class RSISignal(BaseSignal):
    ...
    def compute(self, prices: pd.DataFrame) -> Union[pd.Series, pd.DataFrame]:
        delta = prices.diff()
        gain = (delta.where(delta > 0, 0)).rolling(window=self.lookback, min_periods=self.lookback // 2).mean()
        loss = (-delta.where(delta < 0, 0)).rolling(window=self.lookback, min_periods=self.lookback // 2).mean()
...
class SMACrossoverSignal(BaseSignal):
    ...
    def compute(self, prices: pd.DataFrame) -> Union[pd.Series, pd.DataFrame]:
        fast_sma = prices.rolling(self.fast_period, min_periods=self.fast_period // 2).mean()
        slow_sma = prices.rolling(self.slow_period, min_periods=self.slow_period // 2).mean()
...
class SignalBlender:
    ...
    def __init__(self, signals: List[BaseSignal], weights: Optional[List[float]] = None):
