    ...
    def to_dict(self) -> dict:
        return {
            ...
            "prediction_correct": (
                ...
                if self.predicted_return and self.actual_return
                else None
            ),
            "prediction_error": (
...
class ComparisonView:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        high_conf = [
            c for c in valid if c.signal_confidence and c.signal_confidence > 0.7
        ]
        low_conf = [
            c for c in valid if c.signal_confidence and c.signal_confidence <= 0.3
        ]
        ...
        factor_summary = {f: np.mean(v) for f, v in factor_impact.items()}
        ...
        sentiment_correct = sum(
            ...
            and c.actual_return
            and (
                (c.news_sentiment == "positive" and c.actual_return > 0)
                or (c.news_sentiment == "negative" and c.actual_return < 0)
            )
        )
        ...
        return {
            ...
            # Win/loss breakdown
            "predicted_wins": sum(
                1 for c in valid if c.predicted_return and c.predicted_return > 0
            ),
            "actual_wins": sum(
                1 for c in valid if c.actual_return and c.actual_return > 0
            ),
            ...
            for signal_name, signal_value in c.entry_signals.items():
                ...
                if c.actual_return is not None:
                    ...
                    if c.predicted_return is not None:
                        stats["total_error"].append(
                            c.actual_return - c.predicted_return
                        )
