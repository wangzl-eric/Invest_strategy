...
def compute_momentum(df: pd.DataFrame, period: int = 20, price_col: str = "close") -> pd.Series:
...
def compute_max_drawdown(df: pd.DataFrame, period: int = 60, price_col: str = "close") -> pd.Series:
...
def compute_price_to_sma(df: pd.DataFrame, period: int = 50, price_col: str = "close") -> pd.Series:
...
def compute_zscore(df: pd.DataFrame, period: int = 20, price_col: str = "close") -> pd.Series:
...
class FeaturePipeline:
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature_name in self.features:
            if feature_name == "zscore_20":
                result["zscore_20"] = compute_zscore(df, period=20, price_col=self.price_col)
            elif feature_name == "zscore_60":
                result["zscore_60"] = compute_zscore(df, period=60, price_col=self.price_col)
            elif feature_name == "vol_regime":
                result["vol_regime"] = compute_volatility_regime(df, price_col=self.price_col)
                ...
                result["atr_14"] = compute_atr(
                    df, period=14,
                    high_col=self.high_col, low_col=self.low_col, close_col=self.price_col
                )
    ...
    def list_available_features(self) -> List[str]:
        ...
        all_features.extend([
            "zscore_20", "zscore_60", "vol_regime", "rsi_14", "atr_14",
            "macd", "bollinger_bands", "stochastic"
        ])
...
def compute_features(
    ...
    for feature_name in feature_names:
        ...
        if feature_name.startswith("momentum"):
            ...
            result[feature_name] = compute_volatility(period=period, annualize=annualize, **params)
