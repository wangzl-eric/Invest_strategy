...
| **Event Annotations** | None | ✅ Delivered - FOMC vertical lines on charts, drawdown analyzer with NewsAPI correlation, IBKR news API pipeline, forward-pass dual-tracking (prediction vs actual) | — |
...
| PnL attribution | Asset-level | ✅ Factor-level + decision attribution + forward-pass dual tracking (prediction vs actual comparison) | **P1** |
