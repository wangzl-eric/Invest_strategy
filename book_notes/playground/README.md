...
Model defaults are summarized in `PLAYGROUND_TEAM_MODELS.md`. Persistent defaults live in `workstation/playground/agents/*.md` frontmatter (`runtime:` and `model:`). One-off launch overrides use env vars like `PLAYGROUND_CEREBRO_MODEL=sonnet` or `PLAYGROUND_EXPLORER_RUNTIME=claude`.
