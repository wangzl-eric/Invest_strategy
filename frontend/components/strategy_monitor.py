...
def build_strategy_monitor_layout(data: dict) -> html.Div:
    ...
    if not data:
        return html.Div(
            [
                ...
                html.P(
                    "Run the strategy runner to submit paper orders. Data will appear here.",
                    style={"color": MUTED},
                ),
            ],
    ...
    for p in positions[:20]:
        ...
        pos_rows.append(
            html.Tr(
                [
                    html.Td(
                        sym, style={"fontFamily": "JetBrains Mono", "fontWeight": 600}
                    ),
                    ...
                    html.Td(
                        f"{weight_pct:.1f}%",
                        style={"fontFamily": "JetBrains Mono", "color": MUTED},
                    ),
                ]
            )
        )
                ...
                [
                    ...
                    html.Tbody(
                        ...
                        or [
                            html.Tr(
                                html.Td(
                                    "No positions", colSpan=5, style={"color": MUTED}
                                )
                            )
                        ]
                    ),
        ...
        [
            html.Div(
                [
                    ...
                    html.P(
                        f"${total_recent:,.2f}", className=f"metric-value {pnl_class}"
                    ),
                ],
    ...
    for o in orders[:15]:
        status = o.get("status", "")
        status_color = (
            GREEN if status == "filled" else (RED if status == "rejected" else MUTED)
        )
        order_rows.append(
            html.Tr(
                [
                    ...
                    html.Td(
                        o.get("symbol", ""), style={"fontFamily": "JetBrains Mono"}
                    ),
                    html.Td(
                        o.get("side", ""),
                        style={"color": GREEN if o.get("side") == "BUY" else RED},
                    ),
                    html.Td(
                        f"{o.get('quantity', 0):,.2f}",
            ...
            dbc.Table(
                [
                    ...
                    html.Tbody(
                        ...
                        or [
                            html.Tr(
                                html.Td("No orders", colSpan=5, style={"color": MUTED})
                            )
                        ]
                    ),
                ],
        ...
        fill_rows.append(
            html.Tr(
                [
                    ...
                    html.Td(
                        f.get("symbol", ""), style={"fontFamily": "JetBrains Mono"}
                    ),
                    html.Td(
                        f.get("side", ""),
                        style={"color": GREEN if f.get("side") == "BUY" else RED},
                    ),
                    html.Td(
                        f"{f.get('quantity', 0):,.2f}",
                        style={"fontFamily": "JetBrains Mono"},
                    ),
                    html.Td(
                        f"${f.get('fill_price', 0):,.2f}",
                        style={"fontFamily": "JetBrains Mono"},
                    ),
                ]
            )
            ...
            dbc.Table(
                [
                    ...
                    html.Tbody(
                        ...
                        or [
                            html.Tr(
                                html.Td("No fills", colSpan=5, style={"color": MUTED})
                            )
                        ]
                    ),
                ],
    ...
    return html.Div(
        [
            html.H4(
                "Strategy Monitor",
                style={"color": TEXT_PRIMARY, "marginBottom": "1.5rem"},
            ),
            html.P(
                "Active paper strategies, current positions, PnL, and execution audit trail.",
