...
def _find_project_root() -> Path:
    for candidate in [Path.cwd(), *Path.cwd().parents]:
        if (candidate / "core").exists() and (candidate / "alpha_research").exists():
            return candidate
...
from knowledge.shared.viz_helpers import (  # noqa: E402
    plot_distribution,
    plot_regime_overlay,
...
# ## 1. Load Data
#
# `get_prices` is local-first: it reads the Parquet cache and falls back to the
# source API when the local data is missing.

# %% Load local data
spy = get_prices("SPY", start=START_DATE, end=END_DATE)
tlt = get_prices("TLT", start=START_DATE, end=END_DATE)
...
if data.empty:
    raise RuntimeError(
        "No price data found for SPY/TLT locally or from the source API."
    )
