...
and available via `get_prices()` / `get_fred()` in `alpha_research/quant_data/api.py`.
...
> Use `get_fred(series_id, start=...)`, or pass a natural-language alias to
> `get_data(...)` — the resolver maps it to the canonical series.

### Natural-language aliases (via `get_data`)
...
```python
from alpha_research.quant_data.api import get_prices, get_fred, get_data
...
# Macro backdrop
vix = get_data("vix", start="2015-01-01")            # -> VIXCLS
yc  = get_data("yield curve", start="2010-01-01")   # -> T10Y2Y
credit = get_fred(["BAMLH0A0HYM2", "BAMLC0A0CM"], start="2010-01-01")
```
