...
# Real file at {repo}/core/market_data_store.py -> parents[1] == repo root.
_BASE_DIR = Path(__file__).resolve().parents[1] / "data" / "market_data"
...
try:
    ...
    _IBKR_ASSET_TICKERS = {
        ...
        "ibkr_futures": [],  # Requires CME subscription
        "ibkr_options": [],  # Requires options subscription
    }
except ImportError:
...
class MarketDataStore:
    ...
    def pull_fred_data(
        ...
        from core.market_data_service import _get_fred
    ...
    def pull_ibkr_data(
        ...
        async def fetch_all():
            ...
            for ticker in tickers:
                try:
                    ...
                    # For forex, use full pair name (EURUSD), for others use ticker directly
        ...
        try:
            ...
            try:
                ...
                logger.error(f"IBKR fetch error (fallback): {e2}")
                return 0
        except Exception as e:
