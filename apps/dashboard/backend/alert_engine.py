...
class AlertEngine:
            ...
            with get_db_context() as db:
                ...
                try:
                    ...
                    if "alert_rules" not in table_names:
                        logger.debug(
                            "alert_rules table does not exist, skipping alert evaluation"
                        )
                        return
                except Exception:
                ...
                for rule in rules:
                    try:
                        ...
                        logger.error(
                            f"Error evaluating rule {rule.id}: {e}", exc_info=True
                        )
    ...
    def evaluate_rule(self, rule: AlertRule, db: Session):
        ...
        if triggered:
            existing_alert = (
                db.query(Alert)
                .filter(Alert.rule_id == rule.id, Alert.status == "ACTIVE")
                .first()
            )
            ...
            if not existing_alert:
                ...
                logger.info(
                    f"Alert triggered: {rule.name} for account {rule.account_id}"
                )
    ...
    def _evaluate_pnl_threshold(
        ...
        pnl = (
            db.query(PnLHistory)
            .filter(PnLHistory.account_id == rule.account_id)
            .order_by(desc(PnLHistory.date))
            .first()
        )
        ...
        message = (
            f"P&L threshold breached: ${total_pnl:,.2f} (threshold: ${threshold:,.2f})"
        )
        context = {
            "total_pnl": total_pnl,
            "threshold": threshold,
    ...
    def _evaluate_position_size(
        ...
        for pos in positions:
            notional = abs(
                (pos.market_value or 0) or (pos.quantity or 0) * (pos.market_price or 0)
            )
        ...
        message = (
            f"Position size limit exceeded for {len(violating_positions)} position(s)"
        )
        context = {
    ...
    def _evaluate_volatility(
        ...
        pnl_records = (
            db.query(PnLHistory)
            .filter(
                PnLHistory.account_id == rule.account_id, PnLHistory.date >= cutoff_date
            )
            .order_by(PnLHistory.date)
            .all()
        )
    ...
    def _send_notifications(self, alert: Alert, rule: AlertRule, db: Session):
        ...
        channel_ids = [
            cid.strip() for cid in rule.channel_ids.split(",") if cid.strip()
        ]
        ...
        for channel in channels:
            try:
                ...
                logger.error(
                    f"Error sending notification via channel {channel.id}: {e}"
                )
                alert.notification_attempts += 1
