...
class IBKRConfig(BaseSettings):
    ...
    port: int = Field(
        default=7497,
        description="IBKR TWS/Gateway port (7497 for paper, 7496 for live)",
    )
...
class DatabaseConfig(BaseSettings):
    ...
    url: str = Field(
        default="sqlite:///./ibkr_analytics.db", description="Database connection URL"
    )
...
class AppConfig(BaseSettings):
    ...
    update_interval_minutes: int = Field(
        default=15, description="Data update interval in minutes"
    )
...
class FlexQueryConfig(BaseSettings):
    ...
    queries: List[dict] = Field(
        default_factory=list, description="List of Flex Query definitions"
    )

    # Legacy fields for backwards compatibility
    trade_query_id: str = Field(
        default="", description="Query ID for trade history (deprecated)"
    )
    position_query_id: str = Field(
        default="", description="Query ID for positions (deprecated)"
    )
    activity_query_id: str = Field(
        default="", description="Combined activity query ID (deprecated)"
    )
    ...
    def is_configured(self) -> bool:
        ...
        return bool(
            self.token
            and (self.queries or self.trade_query_id or self.activity_query_id)
        )
    ...
    def get_all_queries(self) -> List[FlexQueryDefinition]:
        ...
        if not result:
            ...
            if self.trade_query_id and self.trade_query_id != self.activity_query_id:
                result.append(
                    FlexQueryDefinition(
                        id=self.trade_query_id, name="Trade History", type="trades"
                    )
                )
            if self.position_query_id and self.position_query_id not in [
                self.activity_query_id,
                self.trade_query_id,
            ]:
                result.append(
                    FlexQueryDefinition(
                        id=self.position_query_id, name="Positions", type="positions"
                    )
                )
...
class LLMConfig(BaseSettings):
    """LLM (Qwen/DashScope) configuration for attribution."""
    ...
    qwen_base_url: str = Field(
        ...
        description="API base URL (use dashscope-intl for Singapore)",
    )
    qwen_max_tokens: int = Field(default=1000, description="Max tokens in response")
    ...
    class Config:
        env_prefix = ""
        env_mapping = {"qwen_api_key": "QWEN_API_KEY"}
...
class DuckDBConfig(BaseSettings):
    ...
    read_parquet_direct: bool = Field(
        default=True, description="Read Parquet directly without copying"
    )
...
class MLflowConfig(BaseSettings):
    ...
    experiment_name: str = Field(
        default="quant_research", description="Default experiment name"
    )
        ...
        if config_path.exists():
            ...
            if "data_sources" in config_data:
                ...
                if "fallback" in ds_cfg:
                    settings.data_sources_fallback = DataSourceFallback(
                        **ds_cfg["fallback"]
                    )
