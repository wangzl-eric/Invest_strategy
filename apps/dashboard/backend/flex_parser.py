...
class FlexParser:
    ...
    def parse_directory(
        self, directory: str, pattern: str = "**/*.csv"
    ) -> List[FlexParseResult]:
    ...
    def get_consolidated_performance(self) -> pd.DataFrame:
        ...
        if "symbol" in df.columns:
            df = df[
                df["symbol"].notna() & (df["symbol"] != "") & (df["symbol"] != "Symbol")
            ]
        ...
        for section_name, (start_line, end_line) in sections.items():
            ...
            try:
                ...
                result.parse_errors.append(
                    f"Error parsing section {section_name}: {str(e)}"
                )
    ...
    def _process_account_summary(
        self, df: pd.DataFrame, result: FlexParseResult
    ) -> pd.DataFrame:
    ...
    def _process_trades(
        ...
        executions = executions.rename(
            columns={k: v for k, v in rename_map.items() if k in executions.columns}
        )

        # Parse dates
        if "datetime_str" in executions.columns:
            executions["trade_datetime"] = executions["datetime_str"].apply(
                self._parse_datetime_with_time
            )
        if "trade_date_str" in executions.columns:
            executions["trade_date"] = executions["trade_date_str"].apply(
                self._parse_date
            )

        # If trade_date is missing, extract from trade_datetime
        if (
            "trade_date" not in executions.columns
            and "trade_datetime" in executions.columns
        ):
        ...
        if "proceeds" in executions.columns and "commission" in executions.columns:
            executions["net_proceeds"] = (
                executions["proceeds"] + executions["commission"]
            )
