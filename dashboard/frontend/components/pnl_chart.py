...
def create_pnl_chart(pnl_data):
    ...
    if not pnl_data:
        ...
        return html.Div(
            [
                html.P(
                    "No PnL data available. Please fetch data first:",
                    className="text-muted mb-2",
                ),
                html.P(
                    "1. Go to http://localhost:8000/docs", className="text-muted mb-1"
                ),
                html.P(
                    "2. Find the /api/fetch-data endpoint", className="text-muted mb-1"
                ),
                html.P(
                    "3. Click 'Try it out' and 'Execute'", className="text-muted mb-1"
                ),
